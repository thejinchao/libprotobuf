...
build_intermediate     = "_ps5_tmp"
protobuf_src_path   = "../../protobuf-source"
libprotobuf_path    = "../../libprotobuf"
install_path         = "_install"

def create_vs_prj():
	ps5_cmake=os.path.join(os.getenv("SCE_ROOT_DIR"), "Prospero", "Tools", "CMake", "PS5CMake.bat")
	if(not os.path.isfile(ps5_cmake)):
		print("Can't find PS5CMake.bat '%s'")
		return -1;
	print("Find PS5 CMake batch file '%s'" % ps5_cmake)
	cmd_line = [ps5_cmake]
...
def copy_library():
	...
	target_lib = os.path.join(libprotobuf_path, "lib/ps5");
	if(os.path.exists(target_lib)):
		shutil.rmtree(target_lib, True)
	os.makedirs(target_lib)
	shutil.copy(os.path.join(install_path, "lib/libprotobuf.a"), os.path.join(libprotobuf_path, "lib/ps5/libprotobuf.a"))
